...
def get_mbid_of_all_rows(df, network):
    ...
    for index, rows in df.iterrows():
        ...
        try:
            ...
            track_published_date = "No Published Date"

        try:
            artist_mbid = artist.get_mbid()
        except AttributeError as error:
            artist_mbid = "No Artist"
...
def get_spotipy_id(df, cid =None, secret = None ):
...
def get_spotify_data(df, batchsize_value = 100, cid = None, secret = None  ):
